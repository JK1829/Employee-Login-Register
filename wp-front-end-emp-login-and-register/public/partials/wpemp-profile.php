   ...
   <div class="user_img2">
      <div class="u_pro">
         <?php 
            ...
            $profile_pic = get_user_meta($current_user->ID,'wpemp_profile_pic',true);
...
<ul class="nav nav-tabs text_color2">
      ...
      <div class="tab-content register_r">
         <div id="home" class="tab-pane fade in active">
            <br>           
            <form name="wpempProfileForm" id="wpempProfileForm" method="post" enctype="multipart/form-data">
               <div id="wpemp-profile-loader-info" class="wpemp-loader" style="display:none;">
                  <img src="<?php echo plugins_url('images/ajax-loader.gif', dirname(__FILE__)); ?>"/>
                  <span><?php _e('Please wait ...', $this->plugin_name); ?></span>
               </div>
               <div id="wpemp-profile-alert" class="alert alert-danger" role="alert" style="display:none;"></div>
               <div class="form-group">
                  <label for="firstname"><?php _e('First name', $this->plugin_name); ?></label>
                  <sup class="wpemp-required-asterisk">*</sup>
                  <input type="text" class="form-control re_register" name="wpemp_fname" id="wpemp_fname" value="<?php echo $current_user->first_name; ?>">
               </div>
               <div class="form-group">
                  <label for="lastname"><?php _e('Last name', $this->plugin_name); ?></label>
                  <input type="text" class="form-control re_register" name="wpemp_lname" id="wpemp_lname" value="<?php echo $current_user->last_name; ?>">
               </div>
               <div class="form-group">
                  <label for="email"><?php _e('Email', $this->plugin_name); ?></label>
                  <sup class="wpemp-required-asterisk">*</sup>
                  <input type="text" class="form-control re_register" name="wpemp_email" id="wpemp_email" value="<?php echo $current_user->user_email; ?>">
               </div>
               <div class="form-group">
                  <label for="profile_pic"><?php _e('Profile Image', $this->plugin_name); ?></label>
                  <sup class="wpemp-required-asterisk">*</sup>
                  <input type="file" class="form-control re_register" name="wpemp_profile_pic" id="wpemp_profile_pic">
               </div>
               <?php
                  // this prevent automated script for unwanted spam
                  if (function_exists('wp_nonce_field'))
                      wp_nonce_field('wpemp_profile_action', 'wpemp_profile_nonce');
